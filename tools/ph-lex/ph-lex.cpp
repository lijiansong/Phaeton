...
ParseResult parseArgs(Options &options, int &argc, char **argv) {
  if (argc < 2) {
    std::cerr << PH_LEXER_EXE << ":" << FRED(" error: ")
...
void buildJobs(const Options &options, const ParseResult &result) {
  ...
  if (result.count("input")) {
    ...
    if (!in_ph_file_stream.is_open()) {
      std::cerr << PH_LEXER_EXE << ":" << FRED(" error: ") << "Fail to open "
                << ph_src_file << '\n';
      exit(-1);
    }
  ...
  // If we get here, that means we have to perform Phaeton lexer.
...
int main(int argc, char *argv[]) {
  ...
  Options options(/*argv[0]*/ PH_LEXER_EXE,
